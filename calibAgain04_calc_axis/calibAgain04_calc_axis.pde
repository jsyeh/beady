...
void mousePressed(){
  ...
  N++;
  if(N==3){ //寄生蟲，湊齊3個頂點
    strangeX = new PVector(x[1]-x[0],  y[1]-y[0]);
    strangeY = new PVector(x[2]-x[0],  y[2]-y[0]);
    axis3X = new PVector(gx[1]-gx[0],  gy[1]-gy[0],  gz[1]-gz[0]);
    axis3Y = new PVector(gx[2]-gx[0],  gy[2]-gy[0],  gz[2]-gz[0]);
  }
}
