...
int state=0; //0: TRIANGLE_FAN, 1: TRIANGES/face, 2: Hooke's Law
void draw(){
  background(#FFFFF2);
  if(state==0){
    fill(255,0,0);
  ...
  }else if(state==2){ //畫六邊形 centers[i]
    for(int i=0; i<centersN; i++){
      int i2 = (i+1)%centersN;
      HookeForce( centers[i], centers[i2], HookeLen);
    }
    noFill();
    beginShape();
    for(int i=0; i<centersN; i++){  
...
void keyPressed(){
  ...
  for(int i=0; i<centersN; i++){
    int i2 = (i+1)%centersN;
    total += dist(centers[i].x, centers[i].y, centers[i2].x, centers[i2].y);
  }
  HookeLen = total / centersN;
}
