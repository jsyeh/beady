...
void setup(){
  ...
  axis3Z = axis3X.cross(axis3Y).normalize().mult(axis3X.mag());
}
void draw(){
  perspective();
  background(#FFFFF2);
...
void myTransformEllipse(float oldX, float oldY, float oldZ){
  ...
  */
  ...
  float len = dist(x[0],y[0], x[1],y[1]);
  PVector newV = new PVector(newX, newY, newZ).div(len);
  float newX2 = newV.dot(axis3X);
  float newY2 = newV.dot(axis3Y);
  float newZ2 = newV.dot(axis3Z);
  pushMatrix();
    translate(point3[0].x+newX2, point3[0].y+newY2, point3[0].z + newZ2);
    image(img, 0, 0);
  popMatrix();
  println(newX2, newY2, newZ2);
