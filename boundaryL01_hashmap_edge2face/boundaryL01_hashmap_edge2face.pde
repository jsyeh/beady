...
HashMap<String, Integer> edge2face;
void setup(){
  size(500,500, P3D);
...
void prepareVertexFacesEdges(String [] lines) {
  ...
  edge2face = new HashMap<>(); //edge變出字串，便能查到face
  for (int i=0; i<lines.length; i++) {
    ...
    if (c=='v') {
      ...
      for(int k=0; k<face.size(); k++){
        String edgeVV = face.get(k) + "+" + face.get((k+1)%face.size());
        edge2face.put(edgeVV, faces.size()-1); 
      }
    }
  }
}
